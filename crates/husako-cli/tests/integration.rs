...
/// Write `k8s/meta/v1.js` with chain starter functions (name, namespace, label, annotation).
fn write_meta_v1_module(root: &Path) {
    let meta_dir = root.join(".husako/types/k8s/meta");
...
function _resolveContainers(items) {
...
class _Deployment extends _ResourceBuilder {
  ...
  containers(v) { return this._setDeep("template.spec.containers", _resolveContainers(v)); }
  initContainers(v) { return this._setDeep("template.spec.initContainers", _resolveContainers(v)); }
}
...
import { _ResourceBuilder, _createSpecFragment, _createResourceChain, _createResourceRequirementsChain } from "husako/_base";
...
"#,
    ...
    // k8s/meta/v1.js with metadata chain starters
    write_meta_v1_module(root);
}
...
#[test]
fn missing_build() {
    let f = write_temp_ts(r#"import husako from "husako"; const x = 1;"#);
    husako()
...
#[test]
fn double_build() {
    let f = write_temp_ts(r#"import husako from "husako"; husako.build([]); husako.build([]);"#);
    husako()
        .args(["render", f.path().to_str().unwrap()])
        .assert()
        .code(7)
...
fn strict_json_undefined() {
    let f = write_temp_ts(
        r#"import husako from "husako"; husako.build({ _render() { return { a: undefined }; } });"#,
    );
    husako()
        .args(["render", f.path().to_str().unwrap()])
        .assert()
...
fn strict_json_function() {
    let f = write_temp_ts(
        r#"import husako from "husako"; husako.build({ _render() { return { fn: () => {} }; } });"#,
    );
    husako()
        .args(["render", f.path().to_str().unwrap()])
        .assert()
...
fn render_project_imports() {
    ...
    std::fs::write(
        ...
        r#"import { name } from "k8s/meta/v1";
...
const nginx = Deployment()
  ...
  .resources({ requests: { cpu: "1", memory: "2Gi" }, limits: { cpu: "500m", memory: "1Gi" } });
...
import { Deployment } from "k8s/apps/v1";
const m = name("test").label("a", "1").label("b", "2").label("c", "3");
...
#[test]
#[ignore = "templates use old API — will be updated in Part 4"]
...
import { Deployment } from "k8s/apps/v1";
import { LabelSelector } from "k8s/_common";
// containers can carry resource requirements directly
const d = Deployment()
    .metadata(name("nginx"))
    .replicas(2)
    .selector(LabelSelector().matchLabels({ app: "nginx" }))
...
import husako from "husako";
husako.build([{ _render() { return { apiVersion: "v1", kind: "Namespace", metadata: { name: "out" } }; } }]);
"#;
...
fn write_fixture_plugin(root: &Path, name: &str) {
    // Plugin tests need k8s/meta/v1 for chain starters (name(), namespace(), etc.)
    write_meta_v1_module(root);
...
fn install_flux_plugin(root: &Path) {
    // FluxCD tests need k8s/meta/v1 for chain starters (name(), namespace(), etc.)
    write_meta_v1_module(root);
...
/// - `resources()` accepts `_ResourceRequirementsChain` (returned by `requests()` from `k8s/core/v1`)
fn project_with_typed_k8s(ts_content: &str) -> (tempfile::TempDir, std::path::PathBuf) {
    ...
    std::fs::create_dir_all(types_k8s.join("meta")).unwrap();
...
import { _ResourceBuilder, _SpecFragment, _ResourceChain, _ResourceRequirementsChain } from "husako/_base";
...
export interface Namespace extends _ResourceBuilder {}
...
export function name(v: string): _SpecFragment;
export function image(v: string): _SpecFragment;
export function imagePullPolicy(v: string): _SpecFragment;
export function cpu(v: string | number): _ResourceChain;
export function memory(v: string | number): _ResourceChain;
...
"#,
    ...
    std::fs::write(
        types_k8s.join("meta/v1.d.ts"),
        r#"// Auto-generated by husako. Do not edit.

import { _SpecFragment } from "husako/_base";

export function name(v: string): _SpecFragment;
export function namespace(v: string): _SpecFragment;
export function label(k: string, v: string): _SpecFragment;
export function annotation(k: string, v: string): _SpecFragment;
"#,
    )
    .unwrap();
