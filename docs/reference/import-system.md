...
husako uses a custom module resolver. The supported import specifiers are different from Node.js.
...
Generated by `husako generate`. Each file exports builder factories for every resource kind in that API group and version.
...
The group and version come from the Kubernetes API path. For core resources (`/api/v1`), use `k8s/core/v1`.
...
Generated by `husako generate` when chart dependencies are configured. Exports typed value builders for that chart.

```typescript
import { Values } from "helm/ingress-nginx";

const nginxValues = Values()
  .replicaCount(2)
...
Plugins declare module mappings in their `plugin.toml`. After `husako generate` installs the plugins, their modules are importable by their declared specifiers.
...
By default, resolved paths must stay within the project root (the directory containing `husako.toml`). Use `--allow-outside-root` to override this:
...
husako runs entirely offline after `husako generate`. All dependencies must be local or declared in `husako.toml`.
